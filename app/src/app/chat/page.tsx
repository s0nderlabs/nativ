...
import { createPublicClient, http, webSocket, toHex, encodeFunctionData } from "viem";
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        setAgents(
          addresses.map((addr, i) => ({ address: addr, name: agentData[i].name }))
        );
      } catch {}
    }
    load();
  ...
  if (!address) {
    return (
      <div className="max-w-4xl mx-auto px-6 pt-24 pb-10">
        ...
        <p className="text-muted text-sm">Connect your wallet to message agents</p>
      </div>
    );
  }

  return (
    <div className="max-w-4xl mx-auto px-6 pt-24 pb-10 flex gap-6 h-[calc(100dvh-96px)]">
      {/* Agent list */}
        ...
        <div className="space-y-px">
          {agents.map((agent) => (
            <button
              ...
              className={`w-full text-left text-xs px-3 py-2 transition-[background-color,color] duration-150 ${
          ...
          {agents.length === 0 && (
            <p className="text-muted text-xs">No agents online</p>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col">
        <div className="mb-4">
          ...
          <p className="label-mono mt-1">
            {selectedAgent
              ? `Talking to ${selectedAgent.name}.init`
              : "Select an agent"}
          </p>
        </div>
        ...
        <div ref={messagesRef} className="flex-1 overflow-y-auto space-y-2 mb-4">
          {messages
            ...
            .map((msg) => (
              ...
              >
                <div
                  className={`max-w-[70%] px-3 py-2 text-xs ${
                    msg.direction === "out"
        ...
        {selectedAgent && (
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-surface border border-border px-4 py-3 text-xs text-fg placeholder:text-muted/50 focus:border-border-strong focus:outline-none transition-[border-color] duration-200"
            />
            <button
              onClick={handleSend}
              disabled={sending || !input.trim()}
              className="px-6 py-3 bg-fg text-bg text-xs font-medium hover:opacity-80 disabled:opacity-40 transition-opacity duration-200"
            >
              {sending ? "..." : "Send"}
            </button>
          </div>
