...
export default function ExplorerPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      setLoading(false);
      // After first load, disable entry animations so polls don't re-trigger
      ...
      {agents.length === 0 && !loading ? (
        ...
        <div className="space-y-px">
          {agents.map((agent, i) => (
            <motion.div
              key={agent.address}
              initial={initialLoad.current ? { opacity: 0, y: 10 } : false}
              animate={{ opacity: 1, y: 0 }}
              transition={initialLoad.current ? { duration: 0.5, delay: i * 0.07, ease: [0.16, 1, 0.3, 1] } : { duration: 0 }}
              className="group flex items-center px-4 py-3 border border-transparent hover:border-border-strong hover:bg-surface/50 transition-[border-color,background-color] duration-200 will-change-[transform,opacity]"
            >
              {/* Avatar */}
